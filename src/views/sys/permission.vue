    ...
    >
      ...
      <el-table-column label="权限名" align="left" min-width="250px">
        <template slot-scope="{row}">
          <el-tag v-if="row.filterUrl.split('/').length===2" type="success">{{ row.permission }}</el-tag>
          <el-tag v-if="row.filterUrl.split('/').length>2" type="info">{{ row.permission }}</el-tag>
        </template>
      </el-table-column>
