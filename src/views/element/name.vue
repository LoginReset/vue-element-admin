<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column label="创建时间" align="center" sortable="custom" prop="createDate" width="220">
        <template slot-scope="{row}">
      ...
      <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible">
        <el-form
          ...
          <el-form-item label="元件名称" prop="elementName">
            <el-input v-model.trim="temp.elementName" clearable placeholder="请输入元件名称" />
          </el-form-item>
...
<script>
import {getElView,postElAdd,postElUp,getElDel} from '@/api/ele'
...
export default {
  ...
  methods:{
    getList(){
      ...
      getElView(this.listQuery).then(response=>{
    ...
    updateData(){
      ...
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          postElUp(this.temp).then(response => {
              this.dialogFormVisible = false
              this.$notify({
              title: '成功',
              message: '更新成功',
    ...
    handleDelete(row,index){
      this.$confirm('确定删除元件名称?', '警告', {
        ...
        getElDel(requestData).then(response => {
          this.$notify({
            title: '成功',
            message: '删除成功',
            type: 'success',
