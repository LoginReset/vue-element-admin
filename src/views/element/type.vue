      ...
      <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible">
        <el-form
          ...
          <el-form-item label="元件类型" prop="typeName">
            <el-input v-model.trim="temp.typeName" clearable placeholder="请输入元件类型" />
          </el-form-item>
...
<script>
import {getTypeView,postTypeAdd,postTypeUp,getTypeDel} from '@/api/ele'
...
export default {
  ...
  data(){
    ...
    return{
      ...
      rules:{
        typeName:[{ required: true, message: '元件类型必填', trigger: 'change' }],
  ...
  methods:{
    getList(){
      ...
      getTypeView(this.listQuery).then(response=>{
    ...
    createData(){
      ...
      if(valid){
        postTypeAdd(this.temp).then(response=>{
    ...
    updateData(){
      ...
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          postTypeUp(this.temp).then(response => {
              this.dialogFormVisible = false
              this.$notify({
              title: '成功',
              message: '更新成功',
    ...
    handleDelete(row,index){
      this.$confirm('确定删除元件类型?', '警告', {
        ...
        getTypeDel(requestData).then(response => {
          this.$notify({
            title: '成功',
            message: '删除成功',
            type: 'success',
