  ...
  <div class="app-container">
    ...
    >
      <el-table-column
        ...
        width="50"
      />
      <el-table-column label="注册数量" align="center" width="220">
      ...
      <el-table-column label="批量注册编号" align="center">
        <template slot-scope="{row}">
          <el-tooltip class="item" effect="dark" content="点击查看设备" placement="top">
            <span @click="searchDevice(row.registerNum)" class="span">{{ row.registerNum }}</span>
          </el-tooltip>
        </template>
      </el-table-column>
    ...
    <el-dialog title="设备查看" :visible.sync="dialogFormVisible" width="85%"> 
        ...
        >
          ...
          <!-- <el-table-column label="批量注册编号" align="center" width="220" show-overflow-tooltip>
            <template slot-scope="{row}">
              <span>{{ row.registerNum }}</span>
            </template>
          </el-table-column> -->
          <el-table-column label="生产编号" align="center" width="220" show-overflow-tooltip>
            <template slot-scope="{row}">
              <span>{{ row.productNum }}</span>
            </template>
          </el-table-column>
          <!-- <el-table-column label="设备昵称" align="center" width="220" show-overflow-tooltip>
            <template slot-scope="{row}">
          ...
          <el-table-column label="最新版本" align="center" show-overflow-tooltip>
            <template slot-scope="{row}">
              <span v-if="row.targetVersion">{{ row.targetVersion }}</span>
            </template>
          </el-table-column>
          <el-table-column label="升级状态" align="center" show-overflow-tooltip>
            <template slot-scope="{row}">
              <span v-if="row.upgradeStatus">{{ row.upgradeStatus }}</span>
            </template>
          </el-table-column>
          ...
          <el-table-column label="更新时间" align="center" prop="create_date" width="220">
            <template slot-scope="{row}">
